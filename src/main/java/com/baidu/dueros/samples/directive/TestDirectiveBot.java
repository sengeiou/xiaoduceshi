...
public class TestDirectiveBot extends BaseBot {
    ...
    protected Response onInent(IntentRequest intentRequest) {
        ...
        if ("test_Hint".equals(intentRequest.getIntentName())) {
            ...
            hints.add("提示2");
            hints.add("提示3");
            com.baidu.dueros.data.response.directive.display.Hint hint = new Hint(hints);
        ...
        if ("test_RenderTemplate".equals(intentRequest.getIntentName())) {
            com.baidu.dueros.data.response.directive.display.templates.ListTemplate2 listTemplate = new ListTemplate2();
            listTemplate.setTitle("title");
            listTemplate.setToken("token");
            // 设置模版列表数组listItems其中一项，即列表的一个元素
            com.baidu.dueros.data.response.directive.display.templates.ListItem listItem = new ListItem();
            ...
            // 定义RenderTemplate指令
            com.baidu.dueros.data.response.directive.display.RenderTemplate renderTemplate = new RenderTemplate(listTemplate);
        ...
        if ("test_Context".equals(intentRequest.getIntentName())) {
            ...
            com.baidu.dueros.data.response.directive.display.templates.BodyTemplate1 bodyTemplate1 = (BodyTemplate1) template;
        ...
        if ("test_playerinfo".equals(intentRequest.getIntentName())) {
            // 新建Play指令
            com.baidu.dueros.data.response.directive.audioplayer.Play play = new Play("http://www.music");
            play.setPlayBehavior(AudioPlayerDirective.PlayBehaviorType.ENQUEUE);
            // 音频播放器的主界面
            com.baidu.dueros.data.response.directive.audioplayer.PlayerInfo playerInfo = new PlayerInfo();
            // 创建按钮
            com.baidu.dueros.data.response.directive.audioplayer.control.PlayPauseButton playpause = new PlayPauseButton();
            com.baidu.dueros.data.response.directive.audioplayer.control.PreviousButton previous = new PreviousButton();
        ...
        if ("test_audiostop".equals(intentRequest.getIntentName())) {
            // 新建stop指令
            com.baidu.dueros.data.response.directive.audioplayer.Stop stop = new Stop();
    ...
    protected Response onElementSelectedEvent(ElementSelectedEvent elementSelectedEvent) {
        com.baidu.dueros.data.response.card.StandardCard card = new com.baidu.dueros.data.response.card.StandardCard();
    ...
    protected Response onLinkClickedEvent(LinkClickedEvent linkClickedEvent) {
        com.baidu.dueros.data.response.card.StandardCard card = new StandardCard();
